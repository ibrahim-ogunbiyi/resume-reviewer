...
class ModelClass:
    ...
    def warm_phrase_extraction_endpoint(cls) -> None:
        _ = requests.post(
            settings.KEYWORD_EXTRACTION_SERVICE_URL,
            json={"text": "Dummy testing to warm up endpoint"},
        )

    @classmethod
    def warm_phrase_encoding_endpoint(cls) -> None:
        _ = requests.post(
            settings.ENCODING_SERVICE_URL, json={"text": "Dummy testing to warm up endpoint"}
        )
