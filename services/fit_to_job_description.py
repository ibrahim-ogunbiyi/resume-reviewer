...
def extract_phrases(text: str, ngram_range: tuple, top_n: int) -> list[str]:
    keybert_model = ModelClass().keybert_model

    key_phrases = keybert_model.extract_keywords(
        docs=text, keyphrase_ngram_range=ngram_range, top_n=top_n
    )
...
def ats_checker(job_description: str, resume_text: str) -> tuple[int, dict]:
    # initialize model
...
def fit_to_job_description(file: FileSchema, job_description: str) -> tuple[dict, tuple[int, dict]]:
